...
def _format_exc(e: Exception) -> str:
    """Devuelve <Tipo: msg @ archivo:linea en funcion> para logs."""
    try:
        tb = e.__traceback__
        last = traceback.extract_tb(tb)[-1] if tb else None
        if last:
            return f"{type(e).__name__}: {e} @ {last.filename}:{last.lineno} in {last.name}"
...
# =========================
# Parseo robusto de salida del LLM (mismo nombre/estructura)
# =========================
...
def _parse_llm_json(raw: str) -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(raw, str):
        print("⚠️ _parse_llm_json: raw no es str", flush=True)
        return None
    ...
    try:
        ...
        print(f"⚠️ _parse_llm_json: json.loads(raw) falló -> {_format_exc(e)}", flush=True)
    ...
    try:
        ...
        print(f"⚠️ _parse_llm_json: limpieza previa falló -> {_format_exc(e)}", flush=True)
    ...
    try:
        ...
        print(f"⚠️ _parse_llm_json: json.loads(cand) falló -> {_format_exc(e)}", flush=True)
    ...
    try:
        ...
        print(f"⚠️ _parse_llm_json: literal_eval/normalización falló -> {_format_exc(e)}", flush=True)
        return None
...
def _worker_un_caso(
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ _worker_un_caso: fallo al imprimir inicio -> {_format_exc(e)}", flush=True)
        ...
        if not obs_unidas:
            ...
            if verbose:
                print(f"⚠️ Caso idx={idx + 1} sin observaciones.", flush=True)
                print(f"🧾 Final [{idx + 1}]: {json.dumps(out, ensure_ascii=False)}", flush=True)
            return out
  ...
  "confianza": "baja|media|alta"
        ...
        for attempt in range(max_retries + 1):
            try:
                if verbose:
                    print(f"🚀 LLM calidad idx {idx + 1} intento {attempt + 1}/{max_retries + 1}", flush=True)
                ...
                if verbose:
                    ...
                    print(f"📥 LLM RAW [{idx + 1}, intento {attempt + 1}]: {preview}", flush=True)
                ...
                if attempt < max_retries:
                    ...
                    if verbose:
                        print(f"⏳ JSON inválido/erróneo ({_format_exc(e)}). Reintentando en {wait_s:.1f}s...", flush=True)
                    import time as _time
                    _time.sleep(wait_s)
                else:
                    if verbose:
                        print(f"⚠️ Reintentos agotados en idx {idx + 1}. Motivo: {_format_exc(e)}", flush=True)
                    llm_json = None
        ...
        if verbose:
            ...
            print(f"✅ Procesado caso {idx + 1} en {elapsed:.2f}s", flush=True)
            print(f"🧾 Final [{idx + 1}]: {json.dumps(out, ensure_ascii=False)}", flush=True)
        return out

    except Exception as e:
        if verbose:
            print(f"❌ Error catastrófico en idx {idx + 1}: {_format_exc(e)}", flush=True)
...
# =========================
# Orquestación (mismos nombres/firmas)
# =========================
...
def procesar_calidad_por_caso(
    ...
    print(f"⚙️ Casos: {n} | hilos={workers} | lotes={total_lotes} | tamaño_lote≈{chunk_size_eff}", flush=True)
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                ...
                print(f"❌ Futuro falló idx={idx}: {_format_exc(e)}", flush=True)
                print("--- TRACEBACK ORQ ---", flush=True)
                print(traceback.format_exc(), flush=True)
                print("---------------------", flush=True)
                res = {
                    "numero_aviso": None,
            ...
            while next_mark_idx < len(marks) and completed >= marks[next_mark_idx]:
                ...
                print(f"⏱️ Lote {lote_num}/{total_lotes} completado ({completed}/{n}) en {elapsed:.2f} s", flush=True)
                lot_start_time = time.perf_counter()
                next_mark_idx += 1

    total_elapsed = time.perf_counter() - start_global
    print(f"✅ Finalizado. Registros: {n} — Tiempo total: {total_elapsed:.2f} s", flush=True)
